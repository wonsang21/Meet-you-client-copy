...
function DetailsScreen({ route }: Props) {
    ...
    return (
        ...
        <View style={{ flex: 1, alignItems: 'center', justifyContent: 'center' }}>
            <StylePhoto source={{ uri: route.params.user.profile_photo}}></StylePhoto>
                <UserNameAge>{route.params.user.username}, {route.params.user.nickname}, {route.params.user.age}</UserNameAge>
                <UserbloodAndaddress>{route.params.user.address}, {route.params.user.blood}</UserbloodAndaddress>
